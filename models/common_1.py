...
class Conv2d_CG(nn.Conv2d):
    def __init__(self, in_channels=102, out_channels=102, kernel_size=3, stride=1, padding=1, dilation=1, groups=1, bias=True):
        ...
        self.weight_conv = Parameter(torch.zeros(out_channels, in_channels, kernel_size, kernel_size), requires_grad=True)
    ...
    def forward(self, x):
        ...
        if self.ind:
            ...
            if self.g > 3:
                # out2 = self.ci_bn2(ce2)
                # out2 = out2.view(b, c//self.g, self.g, -1).transpose(2, 3)
                # out2 = self.act(out2)
                # oc = self.ci(out2).transpose(2, 3).contiguous()
            ...
            return torch.matmul(out, x_un).view(b, self.oc, int(128), int(ll))
