...
def linear_attention(q, k, v):
    k_cumsum = k.sum(dim = -2)#(1,1,128)
    D_inv = 1. / torch.einsum('...nd,...d->...n', q, k_cumsum.type_as(q)) #(1,1,18432)
    context = torch.einsum('...nd,...ne->...de', k, v)  #(1,1,128,128)
...
class ENLA(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        if self.no_projection:
            ...
            create_kernel = partial(softmax_kernel, projection_matrix = self.projection_matrix, device = device) #偏函数(128,32)
            q = create_kernel(q, is_query = True)#(1,1,128*144,32) (128,32)--(1,18432,128)
            k = create_kernel(k, is_query = False)#(1,1,128*144,32) (128,32)--(1,18432,128)
